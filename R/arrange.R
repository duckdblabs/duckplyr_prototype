...
arrange.data.frame <- function(.data, ..., .by_group = FALSE) {
  arrange.duckdb_relation(duckdb::rel_from_df(get_default_duckdb_connection(), .data), ..., .by_group)  
}


#' @export
arrange.duckdb_relation <- function(.data, ..., .by_group = FALSE) {
  dots <- dplyr_quosures(...)
  order_expr <- lapply(dots, function(q) {
    duckdb_encode_expression(rlang::quo_get_expr(q), rlang::quo_get_env(q), .data)
  })
  duckdb::rel_order(.data, order_expr)
}
