...
distinct.data.frame <- function(.data, ..., .keep_all = FALSE) {
  distinct.duckdb_relation(duckdb::rel_from_df(get_default_duckdb_connection(), .data), ..., .keep_all=.keep_all)
}

#' @export
distinct.duckdb_relation <- function(.data, ..., .keep_all = FALSE) {
    stopifnot(missing(.keep_all))
  duckdb::rel_distinct(.data)
}
