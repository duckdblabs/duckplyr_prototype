...
group_by.data.frame <- function(.data, ..., .add = FALSE, .drop = group_by_drop_default(.data)) {
  group_by.duckdb_relation(duckdb::rel_from_df(get_default_duckdb_connection(), .data), ..., .add, .drop)  
}


#' @export
group_by.duckdb_relation <- function(.data, ..., .add = FALSE, .drop = group_by_drop_default(.data)) {
  dots <- dplyr_quosures(...)
  group_expr <- lapply(dots, function(q) {
    duckdb_encode_expression(rlang::quo_get_expr(q), rlang::quo_get_env(q), .data)
  })
  attr(.data, "grouping") <- group_expr
  .data
}
