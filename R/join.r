...
inner_join.data.frame <- function(x, y, by = NULL, copy = FALSE,
    ...
    if (!inherits(x, "duckdb_relation")) {
      x <- duckdb::rel_from_df(get_default_duckdb_connection(), x)
    }
    if (!inherits(y, "duckdb_relation")) {
      y <- duckdb::rel_from_df(get_default_duckdb_connection(), y)
    }

  inner_join.duckdb_relation(x, y, by, copy, suffix, keep, na_matches)
}

#' @export
inner_join.duckdb_relation <- function(x, y, by = NULL, copy = FALSE, suffix = c(".x", ".y"), ...,
                                                                         keep = FALSE,
                                                                         na_matches = c("na", "never")) {
    if (!inherits(y, "duckdb_relation")) {
      y <- duckdb::rel_from_df(get_default_duckdb_connection(), y)
    }
# todo support nameless join col names for USING-style join

# we need to set some names to make the join condition non-ambigous
# TODO investigate if there are better options
   x <- duckdb::rel_set_alias(x, paste0("inner_join_lhs_", sample(1:10^7, 1)))
   y <- duckdb::rel_set_alias(y, paste0("inner_join_rhs_", sample(1:10^7, 1)))

stopifnot(!keep)

    cond <- list()
    for (i in 1:length(by)) {
        cond <- c(cond, duckdb::expr_function("==", list(duckdb::expr_reference(names(by)[i], x), duckdb::expr_reference(by[i], y))))
    }
   rel <- duckdb::rel_inner_join(x, y, cond)


# TODO deal with duplicate column names which are not joined on
   proj <- lapply(names(x), duckdb::expr_reference, x)
    for (col in names(y)) {
        if (col %in% by) {
            next
        }
        proj <- c(proj, duckdb::expr_reference( col, y))
    }
    duckdb::rel_project(rel, proj)
}
