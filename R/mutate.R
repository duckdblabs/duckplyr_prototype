...
mutate.data.frame <- function(.data,
                              ...
                              .before = NULL,
                              .after = NULL) {
  stopifnot(missing(.keep))
  stopifnot(missing(.before))
  stopifnot(missing(.after))
...
#   dplyr_col_select(out, cols_retain)
  mutate.duckdb_relation(duckdb::rel_from_df(get_default_duckdb_connection(), .data), ...)
}



#' @export
mutate.duckdb_relation <- function(.data, ...) {
  dots <- dplyr_quosures(...)

  cols <- lapply(names(.data), duckdb::expr_reference, .data)
  select_expr <- list()
  # TODO pretty ugly still this alias business
  for (i in 1:length(dots)) {
       q <- dots[[i]]
       expr <- duckdb_encode_expression(rlang::quo_get_expr(q), rlang::quo_get_env(q), .data)
       if (!is.null(names(dots))) {
        duckdb::expr_set_alias(expr, names(dots)[i])
       }
       select_expr <- c(select_expr, expr)
  }

  duckdb::rel_project(.data, c(cols, select_expr))
}
