...
summarise.data.frame <- function(.data, ..., .groups = NULL) {
  summarise.duckdb_relation(duckdb::rel_from_df(get_default_duckdb_connection(), .data), ..., .groups)  
}


#' @export
summarise.duckdb_relation <- function(.data, ..., .preserve = FALSE) {
  dots <- dplyr_quosures(...)

  aggregate_expr <- lapply(dots, function(q) {
    duckdb_encode_expression(rlang::quo_get_expr(q), rlang::quo_get_env(q), .data)
    })
    groups <- attr(.data, "grouping")
    if (is.null(groups)) {
    groups <- list()
    }
  duckdb::rel_aggregate(.data, groups, aggregate_expr)
}
